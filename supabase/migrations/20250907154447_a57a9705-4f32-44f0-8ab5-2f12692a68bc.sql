...
CREATE TABLE public.shared_tables (
  ...
  table_id UUID NOT NULL REFERENCES public.table_data(id) ON DELETE CASCADE,
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.table_data td 
    WHERE td.id = table_id 
    AND td.company_id = get_user_company_id()
  )
);
...
CREATE TRIGGER update_shared_tables_updated_at
BEFORE UPDATE ON public.shared_tables
FOR EACH ROW
