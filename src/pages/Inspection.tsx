      ...
      <div className="pt-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="space-y-4">
            {panels.map(panel => <div key={panel.id} className="bg-primary/10 rounded-lg overflow-hidden">
                ...
                {expandedPanels.has(panel.id) && <div className="bg-white text-black p-6">
                    ...
                    {getCategoriesForPanel(panel.id).length > 0 && <div className="space-y-6">
                        {getCategoriesForPanel(panel.id).map(category => <Card key={category.id} className="p-4">
                             <div className="mb-4 cursor-pointer hover:bg-gray-50 rounded p-2 -m-2 transition-colors" onClick={() => toggleCategory(category.id)}>
                               <div className="flex items-center justify-between mb-2">
                                 <div className="flex items-center gap-4">
                                   <div className="flex items-center gap-2">
                                     {expandedCategories.has(category.id) ? <ChevronDown className="h-5 w-5 text-gray-500" /> : <ChevronRight className="h-5 w-5 text-gray-500" />}
                                     {isSuperAdmin ? <DebouncedInput value={category.name} onSave={value => updateCategory(category.id, value)} className="font-medium text-lg max-w-md" placeholder="Category name..." /> : <h3 className="font-medium text-lg">{category.name}</h3>}
                                   </div>
                                 ...
                                 <div className="flex items-center gap-2">
                                   {isSuperAdmin && <>
                                       <Button onClick={() => handleAddEvidence(category.id)} size="sm" className="flex items-center gap-2">
                                         <Plus className="h-4 w-4" />
                                         Add Evidence
                                       </Button>
                                       <Button onClick={() => deleteCategory(category.id)} size="sm" variant="destructive" className="flex items-center gap-2">
                            ...
                            {expandedCategories.has(category.id) && getEvidenceForCategory(category.id).length > 0 && <div className="space-y-2">
                                {/* Grid Header */}
                      ...
                      return <div key={evidenceItem.id} className="grid gap-4 items-start py-2 border-b border-gray-100" style={{
                        gridTemplateColumns: isSuperAdmin ? '2fr 2fr 100px 60px' : '2fr 2fr 100px'
                            ...
                            {expandedCategories.has(category.id) && getEvidenceForCategory(category.id).length === 0 && <div className="text-center py-4 text-muted-foreground text-sm">
