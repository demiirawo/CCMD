  ...
  const fetchMeetings = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('meetings').select('*').order('date', {
        ascending: false
      });
      if (error) {
  ...
  return <div className="min-h-screen bg-gray-100 p-4 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {Object.keys(groupedMeetings).length === 0 ? <Card>
          ...
          return <div key={quarterKey} className="space-y-4">
                    {/* Quarter Header as Link */}
                    <div className="flex items-center justify-between">
                      <Link to={`/reports?quarter=${quarter}&year=${year}`} className="group">
                        <h2 className="text-2xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
                          {quarter} {year}
                          {isCurrent && <Badge variant="outline" className="ml-3 bg-blue-50 text-blue-700 border-blue-200">
                              Current Quarter
                            </Badge>}
                        </h2>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <Dialog>
                                    ...
                                    <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-primary/10">
                                      <DialogHeader>
                                      ...
                                      <div className="space-y-6">
                                        ...
                                        {/* Attendees List */}
                                        {meeting.attendees.length > 0 && <div>
                                            <h3 className="text-lg font-semibold mb-3">Attendees</h3>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                              {meeting.attendees.map((attendee, index) => <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                                                  ...
                                                  <div>
                                                    ...
                                                    {attendee.email && <p className="text-xs text-gray-500">{attendee.email}</p>}
                                                  </div>
                                                </div>)}
                                            </div>
                                          </div>}
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction onClick={() => deleteMeeting(meeting.id)} className="bg-red-600 hover:bg-red-700">
                                          Delete
                                        </AlertDialogAction>
                                      </AlertDialogFooter>
                                    </AlertDialogContent>
