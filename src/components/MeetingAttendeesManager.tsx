...
export const MeetingAttendeesManager = ({ attendees, onChange }: MeetingAttendeesManagerProps) => {
  const addAttendee = () => {
  ...
  const updateAttendee = (id: string, field: 'name' | 'email', value: string) => {
    onChange(attendees.map(attendee => 
  ...
  return (
    <div className="space-y-3">
      {attendees.map((attendee, index) => (
        <div key={attendee.id} className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center">
          <Input
            placeholder="Name"
            value={attendee.name}
