...
export const QuarterlyReportAnalytics: React.FC<QuarterlyReportAnalyticsProps> = ({
  ...
  useEffect(() => {
    ...
    loadData();
  }, [currentCompanyId, type, quarter, year]);

  const loadData = async () => {
    if (!currentCompanyId) return;
    console.log(`🔍 QuarterlyReportAnalytics: Loading ${type} data for company:`, currentCompanyId);
    try {
  ...
  return (
    <Card className="p-4 bg-white" data-chart-type={type}>
      ...
      <ChartContainer config={chartConfig} className="h-64 w-full" data-chart-container={type}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={monthlyData} margin={{ top: 5, right: 5, bottom: 25, left: 5 }}>
