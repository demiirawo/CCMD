...
export interface MeetingStatusItemData {
  id: string;
  title: string;
  status: StatusType;
  lastReviewed: string;
...
interface MeetingStatusItemProps {
  item: MeetingStatusItemData;
...
export const MeetingStatusItem = ({
  item,
  onStatusChange,
  onObservationChange,
  onActionsChange,
  ...
  const handleDateTimeChange = (newDateTime: string) => {
    onObservationChange?.(item.id, newDateTime);
  };

  // Special handling for meeting date - use MeetingDateTimePicker
  const renderObservationField = () => {
    ...
    // Default observation handling for other fields
