...
export const RobustMeetingSummary = ({
  ...
  return <div className="bg-primary/10 rounded-xl shadow-sm outline-none">
      <Card className="w-full bg-white">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <CardTitle className="text-base font-medium text-neutral-950">Meeting Summary</CardTitle>
        ...
        <CardContent>
          <div className="min-h-24">
            {readOnly ? <div className="w-full min-h-24 p-3 text-sm text-foreground bg-gray-50 border border-gray-200 rounded whitespace-pre-wrap break-words overflow-wrap-anywhere">
                {summary || "No meeting summary provided."}
              </div> : isEditing ? <textarea defaultValue={summary} className={cn("w-full min-h-24 p-3 text-sm text-foreground bg-white border rounded resize-none whitespace-pre-wrap", isSaving ? "border-blue-300" : "border-gray-300")} onBlur={e => handleEdit(e.target.value)} onChange={e => handleInput(e.target.value)} onKeyDown={e => {
