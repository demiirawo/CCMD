    ...
    if (showUtilisation) {
      ...
      weeks.forEach(week => {
        ...
        const allocatedStaffIds = new Set(
          serviceUsers.flatMap(u => u.staffAllocations.map(a => a.staffId))
        );
        const unallocatedHours = staff
          .filter(s => !allocatedStaffIds.has(s.id))
    ...
    <Card className="mb-6 border border-border shadow-md">
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">AI Forecast Summary</h3>
          <Button
            variant="outline"
            size="sm"
        ...
        {summary ? (
          ...
          <p className="text-sm text-muted-foreground italic">
            Click "Generate Summary" to create an AI-powered analysis of your {
