...
export const CarePlanAnalytics = () => {
  ...
  const EditableCell = ({ value, onEdit }: { value: number; onEdit: (value: string) => void }) => {
    ...
    return (
      <div 
        ...
        className="h-8 w-16 flex items-center justify-center cursor-pointer hover:bg-muted rounded text-center"
      >
        {value}
      </div>
    );
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
          <EditableInput
            ...
            value={frequencies.high}
            onChange={(value) => handleFrequencyChange('high', value)}
          />
          <EditableInput
            label="Medium Frequency"
            value={frequencies.medium}
            onChange={(value) => handleFrequencyChange('medium', value)}
          />
          <EditableInput
            label="Low Frequency"
            value={frequencies.low}
            onChange={(value) => handleFrequencyChange('low', value)}
          />
          <EditableInput
            label="N/A Frequency"
            value={frequencies.na}
            onChange={(value) => handleFrequencyChange('na', value)}
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          {monthlyData.map((month, index) => (
            <div key={month.month} className="grid grid-cols-6 gap-2 items-center p-2 hover:bg-muted/20 rounded">
              <div className="font-medium">{month.month}</div>
              <div className="flex justify-center">
                <EditableCell
                  value={month.highSU}
                  onEdit={(value) => handleCellEdit(index, 'highSU', value)}
                />
              </div>
              <div className="flex justify-center">
                <EditableCell
                  value={month.mediumSU}
                  onEdit={(value) => handleCellEdit(index, 'mediumSU', value)}
                />
              </div>
              <div className="flex justify-center">
                <EditableCell
                  value={month.lowSU}
                  onEdit={(value) => handleCellEdit(index, 'lowSU', value)}
                />
              </div>
              <div className="flex justify-center">
                <EditableCell
                  value={month.naSU}
                  onEdit={(value) => handleCellEdit(index, 'naSU', value)}
                />
              </div>
              <div className="flex justify-center">
                <EditableCell
                  value={month.completed}
                  onEdit={(value) => handleCellEdit(index, 'completed', value)}
                />
              </div>
            </div>
          ))}
        ...
        <div className="space-y-4">
          ...
          <ChartContainer config={chartConfig} className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={dataWithTargets} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
