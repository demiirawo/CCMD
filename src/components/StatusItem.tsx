...
export const StatusItem = ({
  ...
  return <div className="relative w-full bg-white rounded-xl p-8 mb-3 shadow-md border border-border/30 hover:scale-[1.01] transition-transform duration-300 min-h-[140px]">
      <div className="flex items-start gap-4 w-full">
        <button onClick={() => setIsExpanded(!isExpanded)} className={`flex-shrink-0 p-1 rounded-lg hover:bg-accent/50 transition-colors bg-transparent ${item.title.toLowerCase().includes('audits') || item.title.toLowerCase().includes('staff meeting') || item.title.toLowerCase().includes('call monitoring') ? 'opacity-0 invisible pointer-events-none' : ''}`}>
