...
export const StatusItem = ({
  ...
  return <div className="w-full bg-white rounded-xl p-8 mb-3 shadow-md border border-border/30 hover:scale-[1.01] transition-transform duration-300 min-h-[140px]">
      <div className="flex items-center gap-4 w-full">
        ...
        <div className="flex-[4.6] max-w-0">
          {isEditing ? <textarea defaultValue={item.comment} className="w-full p-3 rounded-lg border border-gray-100 bg-gray-25 resize-none min-h-[100px] text-sm" placeholder="Add your comment..." onBlur={e => handleCommentSubmit(e.target.value)} onKeyDown={e => {
        ...
        }} autoFocus /> : <button onClick={() => setIsEditing(true)} className="w-full text-left p-3 rounded-lg bg-gray-25 hover:bg-gray-50 transition-colors text-sm min-h-[100px] flex items-start border border-gray-100">
              <div className="w-full break-words overflow-wrap-anywhere">{item.comment || "Click to add comment..."}</div>
            </button>}
        </div>
      </div>
